import type { IconFunction as IconComponent, IconOptions } from 'reicon';

/**
 * M3-style adapters for the app's own hand-drawn glyphs.
 *
 * `reicon` icons are functions carrying a `toSvg()`; `CalibrationCurveIcon` is an
 * ordinary React component, and the icon set needs one shape or the call sites have
 * to branch. This wraps the drawing in reicon's exact contract by reusing the
 * component's existing SVG string.
 *
